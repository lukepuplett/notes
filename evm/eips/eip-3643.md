...
#### Transfer
 ...
 Note the 'frozen tokens' idea `_frozenTokens[msg.sender]`, the whitelist registry and Compliance contact.
...
*Note* - It just struck me that this so far talks about compatibility with ERC-20 but not ERC-777 and its hooks. I don't yet understand why the hooking mechanism would not suffice.
...
#### Identity Registry Storage Interface
 ...
 - Abstracts the ID Registry storage implementation.
 ...
 - Allows a single ID Registry contract per token, with its own Trusted Issuers Registry and Claim Topics Registry, but a shared whitelist of investors.
